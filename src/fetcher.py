"""NASDAQ-100 market data client using yfinance + Wikipedia for constituents."""
...
_WIKI_HEADERS = {"User-Agent": "Mozilla/5.0 (compatible; nasdaq-research-agent/0.1)"}


@retry(stop=stop_after_attempt(3), wait=wait_exponential(min=2, max=10))
def _fetch_nasdaq100_constituents() -> pd.DataFrame:
    """Scrape current NASDAQ-100 list from Wikipedia."""
    resp = httpx.get(NASDAQ100_WIKI_URL, headers=_WIKI_HEADERS, follow_redirects=True, timeout=20.0)
    ...
    if "ticker" not in df.columns:
        raise RuntimeError("ticker column not found in NASDAQ-100 Wikipedia table")
    ...
    log.info("fetched %d NASDAQ-100 constituents from Wikipedia", len(df))
    return df[["ticker", "name", "sector", "sub_industry"]]


def _recent_trading_dates(n: int) -> list[str]:
    """Return the last N trading dates by checking QQQ data."""
    end = datetime.now()
...
def _enrich_market_caps(stocks: list[StockMarket]) -> None:
    """Best-effort market cap enrichment via yfinance fast_info."""
